...
\par Subbab ini akan membahas mengenai hasil analisa perangkat lunak \textit{push notification} terpusat.
...
\par Permasalahan yang diangkat pada tugas akhir ini adalah masalah keandalan aplikasi \textit{push notification} terpusat untuk mengirimkan notifikasi ke perangkat pengguna dalam jumlah besar. Berdasarkan hasil pengujian yang dilakukan oleh pengembang sebelumnya, tingkat keberhasilan pengiriman notifikasi ke 3000 pengguna hanya sebesar 63,8 persen \cite{application-thesis}.
\par Berdasarkan analisa yang penulis lakukan, rendahnya tingkat keberhasilan pengiriman disebabkan oleh kasus \textit{race condition} yang tidak ditangani dalam implementasi \textit{message queue} yang telah dibuat. \textit{Race condition} terjadi karena terdapat beberapa \textit{worker} yang mengubah data \textit{queue} secara bersamaan, namun struktur data \textit{linked list} yang digunakan oleh \textit{queue} tidak mendukung perubahan data secara bersamaan \cite{linkedlist-online}. Potongan kode implementasi \textit{message queue} dapat dilihat pada kode sumber~\ref{code:queue_service}.

\subsection{Deskripsi Umum Sistem}
Aplikasi \textit{push notification} terpusat merupakan aplikasi yang digunakan untuk mengirim notifikasi ke perangkat pengguna secara terjadwal dan \textit{asynchronous}, dan pada tugas akhir ini akan dibagi menjadi 3 modul, yaitu \textit{Scheduler}, \textit{Sender APN}, dan \textit{Sender FCM}.
\par Secara umum, \textit{Scheduler} bertanggung jawab untuk penjadwalan antrian notifikasi, \textit{Sender APN} bertanggung jawab untuk mengirimkan notifikasi yang diantrikan ke perangkat \textit{iOS} lewat layanan APNs, dan \textit{Sender FCM} bertanggung jawab untuk mengirimkan notifikasi yang diantrikan ke perangkat \textit{Android} dan \textit{Web} lewat layanan FCM.
...
\begin{longtable}{|p{2cm}|p{7cm}|}
    ...
    \textit{Scheduler} & Membuat dan menjadwalkan pengiriman \textit{packet} notifikasi. \\ \hline
    \textit{Sender APN} & Mengirimkan \textit{packet} notifikasi untuk perangkat \textit{iOS}. \\ \hline
    \textit{Sender FCM} & Mengirimkan \textit{packet} notifikasi untuk perangkat \textit{Android} dan \textit{Web}. \\ \hline
...
\par Diagram kasus penggunaan untuk setiap modul (\textit{Scheduler}, \textit{Sender APN}, dan \textit{Sender FCM}) aplikasi \textit{push notification} terpusat dapat dilihat pada gambar~\ref{diagram_kasus_penggunaan}.
\begin{figure}[H]
...
\par Pada kasus ini, \textit{Scheduler} akan mencari batch yang belum dan boleh dibuatkan \textit{packet} secara berkala. Jika ada, \textit{Scheduler} akan membuatkan \textit{packet} untuk \textit{batch} tersebut.
\tableUcDesc
{UC01}
{Pembuatan Packet}
{Scheduler}
...
\par Pada kasus ini, \textit{Scheduler} akan mencari \textit{packet} yang belum dan sudah waktunya dikirim secara berkala. Jika ada, \textit{Scheduler} akan menambahkan \textit{packet} tersebut ke antrian \textit{Kafka} dengan topik yang dibagi berdasarkan jenis perangkat penerima.
\tableUcDesc
{UC02}
...
\par Pada kasus ini, \textit{Sender APN} akan menunggu \textit{Kafka} mengirimkan packet yang ada di antrian topik "ios". Setelah packet diterima, packet akan dikirimkan ke layanan APNs.
\tableUcDesc
{UC03}
...
\par Pada kasus ini, \textit{Sender FCM} akan menunggu \textit{Kafka} mengirimkan packet yang ada di antrian topik "android" dan "web". Setelah packet diterima, packet akan dikirimkan ke layanan FCM.
\tableUcDesc
{UC04}
...
\par Pada kasus ini, \textit{Scheduler}, \textit{Sender APN}, dan \textit{Sender FCM} dapat menampilkan penggunaan sumber daya (CPU dan Memori) yang digunakan aplikasi dengan menggunakan REST API.
\tableUcDesc
{UC05}
...
\par Pada kasus ini, \textit{Scheduler}, \textit{Sender APN}, dan \textit{Sender FCM} dapat menampilkan status kesehatan aplikasi dan layanan yang terhubung dengan aplikasi dengan menggunakan REST API.
\tableUcDesc
{UC06}
...
\par Pada kasus ini, \textit{Scheduler}, \textit{Sender APN}, dan \textit{Sender FCM} dapat menampilkan konfigurasi aplikasi dengan menggunakan REST API.
\tableUcDesc
{UC07}
...
\par Pada kasus ini, \textit{Scheduler}, \textit{Sender APN}, dan \textit{Sender FCM} dapat menampilkan \textit{log} yang dikeluarkan oleh aplikasi dengan menggunakan REST API.
\tableUcDesc
{UC08}
{Menampilkan Log}
{Scheduler, Sender APN, Sender FCM}
...
\begin{longtable}{|p{2.5cm}|p{2cm}|p{4.5cm}|}
    ...
    User ID & uniqueidentifier & - \\ \hline
    Provider ID & uniqueidentifier & - \\ \hline
    Created At & datetime & Tanggal dan waktu dibuat \\ \hline
    Update At & datetime & Tanggal dan waktu diperbarui \\ \hline
