...
\par \textit{Message queue} pada aplikasi push notification terpusat bekerja dengan cara membuat satu antrian dengan beberapa \textit{thread worker} bekerja sebagai \textit{producer} dan \textit{consumer} dari antrian tersebut secara bersamaan. Berdasarkan analisa yang penulis lakukan, struktur data \textit{linked list} yang digunakan dalam implementasi antrian tidak mendukung perubahan data secara bersamaan \cite{linkedlist-online}. Konsekuensinya, bisa saja terdapat pesan yang gagal ditambahkan ke antrian atau diambil oleh beberapa \textit{thread worker}. Potongan kode implementasi \textit{message queue} dapat dilihat pada Kode Sumber \ref{code:queue_service}.
